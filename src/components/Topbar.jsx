...
const Topbar = () => {
	...
	useEffect(() => {
		const fetchBalance = async () => {
			try {
				const balance = await getBalance(profile?.winery_id);
			...
			<div className="bg-[#F1EDE2] px-4 md:bg-transparent  md:py-3">
				<div className="flex flex-col md:flex-row gap-3 ">
					...
					<div className="px-8 md:px-20 w-calc-full-6rem md:w-full bg-[#F1EDE2]  bg-opacity-70 shadow-xl flex flex-col gap-1 border p-2 md:p-4 rounded-lg items-center flex-1 md:flex-row md:items-center md:justify-between">
						...
						<div className="flex items-center justify-end w-full gap-1 md:w-auto md:gap-1">
							{allNotifications.length ? (
								<div
									className={
										notification.notification
											? "cursor-pointer p-3 rounded-full inline-block text-[#840C4A] hover:transform hover:scale-105 transition-all duration-500"
											: "hidden"
									}
									onClick={handleModal}
								>
